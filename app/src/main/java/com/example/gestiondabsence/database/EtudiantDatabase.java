...
public class EtudiantDatabase extends SQLiteOpenHelper {
    ...
    public Cursor getListEtudiants(String annee,String classe){
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor res =db.rawQuery("select * from "+TABLE_NAME+" WHERE ANNEE= ? AND CLASSE = ? ORDER BY PRENOM, NOM ASC ",new String[] {annee , classe} );
        return res ;
    }
    public boolean updateEtudiant (String exnom , String exprenom
